...
public class InactiveUserJobTest {
    ...
    public void 휴면_회원_전환_테스트() throws Exception {
        ...
        assertEquals(0, userRepository.findByUpdateBeforeAndStatusEquals(LocalDateTime.now().minusYears(1), UserStatus.ACTIVE).size());
    }
}
```  
...
public interface UserRepository  extends JpaRepository<User, Long> {
    ...
    List<User> findByUpdateBeforeAndStatusEquals(LocalDateTime localDateTime, UserStatus status);
}
```
...
...
    ...
    public QueueItemReader<User> inactiveUserReader(){
        List<User> oldUsers =
                userRepository.findByUpdateBeforeAndStatusEquals(LocalDateTime.now().minusYears(1), UserStatus.ACTIVE);
...
```
findByUpdateBeforeAndStatusEquals() 메서드는 현재 날짜 기준 1년 전의 날짜 값과    
...
**QueueItemReader processor 생성 메서드 추가**
```java
package com.community.batch.jobs;
